...
import { Form, FormField } from "@/components/ui/form-hooks";
...
// In summary, the useEffect with form.reset(currentUser) ensures that the form values stay in sync with the currentUser prop, reflecting any updates or changes as they occur.
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSave)} className="space-y-4 sm:space-y-6 bg-gray-100 rounded-lg p-4 sm:p-6 md:p-8 lg:p-10">
                <div>
                    <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">{title}</h1>
                    <FormDescription className="text-sm sm:text-base text-gray-600">
                ...
                <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl>
                            <Input {...field} disabled className="bg-white text-sm sm:text-base" />
                        </FormControl>
                    </FormItem>
                )}></FormField>
                <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl>
                            <Input {...field} className="bg-white text-sm sm:text-base" />
                        </FormControl>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <FormField control={form.control} name="addressLine" render={({ field }) => (
                        <FormItem className="flex-1">
                            <FormLabel className="text-sm sm:text-base font-semibold">Address Line</FormLabel>
                            <Input {...field} className="bg-white text-sm sm:text-base" />
                            <FormMessage className="text-xs sm:text-sm" />
                        </FormItem>
                    )}></FormField>
                    <FormField control={form.control} name="city" render={({ field }) => (
                        <FormItem className="flex-1">
                            <FormLabel className="text-sm sm:text-base font-semibold">City</FormLabel>
                            <Input {...field} className="bg-white text-sm sm:text-base" />
                            <FormMessage className="text-xs sm:text-sm" />
                        </FormItem>
                    )}></FormField>
                    <FormField control={form.control} name="country" render={({ field }) => (
                        <FormItem className="flex-1">
                            <FormLabel className="text-sm sm:text-base font-semibold">Country</FormLabel>
                            <Input {...field} className="bg-white text-sm sm:text-base" />
                            <FormMessage className="text-xs sm:text-sm" />
                        </FormItem>
                    )}></FormField>
                </div>
                {isLoading?<LoadingButton></LoadingButton>:<Button type="submit" className="bg-orange-500 hover:bg-orange-600 text-sm sm:text-base px-4 sm:px-6 py-2 sm:py-3 transition-all duration-300 hover:scale-105">{buttonText}</Button>}
            </form>
        </Form>
    )
}
